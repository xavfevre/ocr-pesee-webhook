...
CARD = r'''        <t t-foreach="range(7)" t-as="i">
          ...
          <div t-attf-class="mh-day t-{{stype or 'vide'}}" t-att-data-date="dstr" t-att-data-type="stype" t-att-data-theo="'%.4f' % s_theo" t-att-data-tmd="fmt(tmd)" t-att-data-tmf="fmt(tmf)" t-att-data-tad="fmt(tad)" t-att-data-taf="fmt(taf)" t-att-data-heures="s and ('%.4f' % s.x_heures) or ''" t-att-data-hs="s and ('%.4f' % s.x_hs) or ''" t-att-data-note="(s and s.x_note) or ''">
            <div class="mh-dh">
              <span class="mh-dname"><t t-esc="jours[i]"/> <t t-esc="d.strftime('%d/%m')"/></span>
              <span class="mh-state" t-att-data-role="'state'">
                <t t-if="s">
                  <t t-if="s.x_type == 'travail'">✓ <t t-esc="('%.2f' % s.x_heures).replace('.', ',')"/> h<t t-if="s.x_h_recup"> + 🔄 <t t-esc="('%.2f' % s.x_h_recup).replace('.', ',')"/> h récup</t><t t-if="s.x_h_sans_solde"> + 🚫 <t t-esc="('%.2f' % s.x_h_sans_solde).replace('.', ',')"/> h sans solde</t><t t-if="s.x_note and (s.x_note.startswith('Récup ') or s.x_note.startswith('Sans solde '))"> (<t t-esc="s.x_note"/>)</t><t t-if="s.x_decouchage"> · 🛏</t></t>
            ...
            <t t-else="">
              ...
              <details class="mh-manque" data-role="manque" style="display:none;">
                ...
                <label>🔄 Heures prises en récup
                  <input type="number" data-f="h_recup" step="any" min="0" max="12" placeholder="0" t-att-value="('%g' % s.x_h_recup) if (s and s.x_type == 'travail' and s.x_h_recup) else ''"/> h
                </label>
                ...
                <label>🚫 Heures sans solde (non payées)
                  <input type="number" data-f="h_ss" step="any" min="0" max="12" placeholder="0" t-att-value="('%g' % s.x_h_sans_solde) if (s and s.x_type == 'travail' and s.x_h_sans_solde) else ''"/> h
                </label>
...
JS = r'''  var LBL = {travail:'✓', cp:'🌴 Congés', maladie:'🤒 Maladie', ferie:'🎉 Férié', absence:'⛔ Absence', recup:'🔄 Récup (journée)', sans_solde:'🚫 Sans solde (journée)', repos:'💤 Repos'};
  ...
  function q4(v){ return Math.round(v*60)/60; }   /* à la minute, comme les horaires */
...
assert 'input[data-f=recup]' not in out and 'ctx.hj_recup = ' not in out
