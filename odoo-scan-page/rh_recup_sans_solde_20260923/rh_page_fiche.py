    ...
    <div class="fs-wrap">
      ...
      <t t-else="">
          ...
          <t t-foreach="lundis" t-as="lundi">
            ...
            <t t-foreach="range(7)" t-as="i">
              ...
              <tr t-attf-class="{{'t-' + typ if typ else ''}} {{'fs-out' if not in_m else ''}}" t-att-data-date="dstr" t-att-data-type="typ" t-att-data-in="'1' if in_m else '0'" t-att-data-theo="'%.4f' % (s.x_theo if solde else theo)" t-att-data-tmd="fmt(tmd)" t-att-data-tmf="fmt(tmf)" t-att-data-tad="fmt(tad)" t-att-data-taf="fmt(taf)" t-att-data-h="('%.4f' % s.x_heures) if s else '0'" t-att-data-hs="('%.4f' % s.x_hs) if s else '0'" t-att-data-r="('%.4f' % s.x_h_recup) if s else '0'" t-att-data-s="('%.4f' % s.x_h_sans_solde) if s else '0'">
                ...
                <td><input type="number" data-f="h_recup" step="any" min="0" max="12" placeholder="0" t-att-value="('%g' % s.x_h_recup) if (s and typ == 'travail' and s.x_h_recup) else ''"/></td>
                <td><input type="number" data-f="h_ss" step="any" min="0" max="12" placeholder="0" t-att-value="('%g' % s.x_h_sans_solde) if (s and typ == 'travail' and s.x_h_sans_solde) else ''"/></td>
        ...
        <div class="fs-recap">
          ...
          <div class="acc" t-att-title="'Solde à récupérer à la veille du mois : arrêté bureau %g h' % (emp.x_recup_solde or 0) + (' au ' + cp_ref.strftime('%d/%m/%Y') if cp_ref else '') + ' + heures comptées depuis'"><t t-if="libre">Solde au <t t-esc="m0.strftime('%d/%m')"/></t><t t-else="">Heures M-1</t><b><t t-esc="('%+.2f' % m_1).replace('.', ',')"/> h</b></div>
