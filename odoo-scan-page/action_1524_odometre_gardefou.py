...
# v4 :
#  - sudo() sur les accès Fleet : un utilisateur bureau sans droit Parc automobile
#    peut modifier la feuille sans erreur d'accès ;
...
for record in records:
    ...
    veh = record.x_vehicle_id.sudo()
    if km and veh:
        cur = veh.odometer or 0.0
        if cur and (km < cur - 1000 or km > cur + 15000):
            continue
        existing = env['fleet.vehicle.odometer'].sudo().search([
            ('vehicle_id', '=', veh.id),
            ('value', '=', km),
        ], limit=1)
        if not existing:
            ...
            env['fleet.vehicle.odometer'].sudo().create(vals)
