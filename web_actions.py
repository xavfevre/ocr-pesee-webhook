...
def executer(call, action_id, ctx):
    return {1987: _rebut, 2055: _parc_intervention,
            ...
            2101: _palettiser, 2102: _scan, 2103: _alerte_palettes,
            2110: _epi_dotation, 2111: _epi_reception, 2112: _epi_annulation,
...
# Le stock physique n'est pas géré chez Maquignon (Xavier, 16/09/2026) : les pierres sont uniques et
# l'important est que chaque commande génère son OF. On ne fait donc plus entrer le stock fini dans
# le colis (l'ancienne action 1585 le faisait) : du stock positif dans un colis pourrait empêcher la
# génération d'un OF. Code conservé derrière ce drapeau.
STOCK_COLIS = False


def _placer_entier(call, of, colis_id):
    """OF terminé posé en entier : lien OF -> colis (+ stock fini dans le colis si STOCK_COLIS)."""
...
def _epi_transfert(call, cfg, pt, src, dst, pr, qty, date, emp_id, note, origin):
    ...
    vals_move = {'description_picking': pr['display_name'], 'product_id': pr['id'], 'product_uom_qty': qty, 'product_uom': pr['uom_id'][0],
